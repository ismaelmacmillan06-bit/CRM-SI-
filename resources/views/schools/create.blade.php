...
<div style="max-width: 800px;">
    <div class="card">
        <div class="card-header">
            <span class="card-title">🏫 Registrar nuevo colegio</span>
                    ...
                    <div class="form-group">
                        ...
                        <select name="city" class="form-control">
                            ...
                            @foreach([
                                ...
                                <option value="{{ $estado }}" {{ old('city') == $estado ? 'selected' : '' }}>
                                    {{ $estado }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex; gap:16px; flex-wrap:wrap; margin-top:4px">
                        ...
                        <label style="display:flex; align-items:center; gap:8px; cursor:pointer;
                                      ...
                                      {{ in_array($level->id, old('levels', [])) ? 'border-color:var(--accent); background:#fff5f6;' : '' }}">
                    ...
                    @error('levels')<small style="color:var(--danger)">{{ $message }}</small>@enderror
                </div>
                    ...
                    <div class="grid-2">
                        <div class="form-group">
                            ...
                            <select name="consultor_digital" class="form-control">
                                ...
                                @foreach($digitales as $c)
                                    <option value="{{ $c->id }}" {{ old('consultor_digital') == $c->id ? 'selected' : '' }}>
                                        {{ $c->user->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="consultor_eca" class="form-control">
                                <option value="">-- Sin asignar --</option>
                                @foreach($ecas as $c)
                                    <option value="{{ $c->id }}" {{ old('consultor_eca') == $c->id ? 'selected' : '' }}>
                                        {{ $c->user->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid-2">
                        <div class="form-group">
                            ...
                            <select name="consultor_elt" class="form-control">
                                <option value="">-- Sin asignar --</option>
                                @foreach($elts as $c)
                                    <option value="{{ $c->id }}" {{ old('consultor_elt') == $c->id ? 'selected' : '' }}>
                                        {{ $c->user->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="representante_ventas" class="form-control">
                                ...
                                @foreach($representantes as $c)
                                    <option value="{{ $c->id }}" {{ old('representante_ventas') == $c->id ? 'selected' : '' }}>
                                        {{ $c->user->name }}
                                    </option>
                                @endforeach
                            </select>
